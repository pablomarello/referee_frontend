    ...
    <div className="bg-green-950 rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300 w-full max-w-3xl mx-auto my-4">
      ...
      <div className="flex justify-between items-center px-6 py-3 border-b border-gray-200 ">
        ...
        <span className={`px-3 py-1 rounded-md text-xs font-bold uppercase ${
             ...
             match.status === 'en progreso' ? 'bg-red-100 text-red-600' : 
             match.status === 'suspendido' ? 'bg-yellow-100 text-yellow-600':
      ...
      <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex flex-col items-center flex-1 w-full">
          <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 font-bold text-xl mb-3">
             {/* Inicial del equipo (o <img /> si tienes logo) */}
             {homeName.charAt(0)}
          </div>
          <h3 className="text-lg font-bold text-white text-center leading-tight">{homeName}</h3>
          <span className="text-xs text-gray-200 font-medium mt-1 uppercase">Local</span>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl px-8 py-4 flex flex-col items-center justify-center min-w-[200px]">
          ...
          <div className="text-4xl font-black text-gray-900 mb-2 tracking-widest">
            {match.score_home ?? 0} <span className="text-gray-400">:</span> {match.score_away ?? 0}
          </div>
          
          {/* Fecha y Hora */}
          <div className="text-center">
            <p className="text-xs text-gray-600 font-medium capitalize">{dateStr}</p>
        ...
        <div className="flex flex-col items-center flex-1 w-full">
          ...
          <h3 className="text-lg font-bold text-white text-center leading-tight">{awayName}</h3>
          <span className="text-xs text-gray-200 font-medium mt-1 uppercase">Visitante</span>
        </div>
      ...
      <div className="px-6 py-4 border-t border-gray-200 bg-gray-50/50 flex items-center text-gray-500 text-sm">
        ...
        <span className="font-medium mr-1 text-black">Cancha: {location}</span> 
